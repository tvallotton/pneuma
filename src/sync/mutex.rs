...
#[derive(Default)]
pub struct Mutex<T> {
    queue: std::sync::Mutex<VecDeque<UThread>>,
    data: std::sync::Mutex<T>,
}

pub struct MutexGuard<'a, T>(ManuallyDrop<std::sync::MutexGuard<'a, T>>, &'a Mutex<T>);
...
impl<T> Mutex<T> {
    ...
    pub fn lock(&self) -> MutexGuard<T> {
        ...
        pneuma::uthread::park_and_release(move || drop(queue));

        loop {
            let mut queue = self.queue.lock().ignore_poison();

            if queue[0].id() != thread_id {
                pneuma::uthread::park_and_release(|| drop(queue));
    ...
    pub fn try_lock(&self) -> Result<MutexGuard<T>, TryLockError> {
        match self.data.try_lock() {
            Ok(value) => Ok(MutexGuard(ManuallyDrop::new(value), self)),
            Err(std::sync::TryLockError::WouldBlock) => Err(TryLockError),
            Err(std::sync::TryLockError::Poisoned(err)) => {
                Ok(MutexGuard(ManuallyDrop::new(err.into_inner()), self))
            }
        }
    }
...
impl<T> Drop for MutexGuard<'_, T> {
    fn drop(&mut self) {
        ...
        let guard = self.1.queue.lock().unwrap_or_else(PoisonError::into_inner);
        let Some(thread) = guard.front() else {
            return;
        };
        thread.unpark();
    }
