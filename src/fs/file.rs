...
/// on closing are ignored by the implementation of `Drop`. These however, can be retrieved
/// by closing explicity with [`File::close`].
///
/// # Implementation
/// On Linux the files are fully asynchronous, taking advantage of the io-uring API's.
/// On
///
/// # Examples
///
...
impl File {
    ...
    ///
    /// ```no_run
    /// // This line could block. It is not recommended to do this on a worker thread.
...
impl Seek for File {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        self.as_std_mut(|fd| fd.seek(pos))
    }
}
