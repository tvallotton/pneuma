...
use crate::{sys::stack::page_size, syscall};
...
impl SignalStack {
    pub fn new() -> io::Result<Self> {
        SignalStack {
            stack: Stack::new(min_sigstack_size() + 256 * 1024)?,
        }
        .install()
    }
...
fn sigsegv_handler(_signum: i32, info: &libc::siginfo_t, _data: *mut ()) {
    ...
    if thread.cx.stack.is_stackoverflow(unsafe { info.si_addr() }) {
        ...
        raw_errln!(
            "error: user-level thread '{name}' has overflowed its stack (stacksize = {size})"
        );
        print_stack_trace();
    } else {
        raw_errln!("error: segmentation fault");
...
fn min_sigstack_size() -> usize {
    ...
    let dynamic_sigstksz = syscall!(getauxval, AT_MINSIGSTKSZ).unwrap();
