...
fn file_test_io_non_positional_read() {
    ...
    {
        let mut rw_stream = File::create(filename).unwrap();
...
fn file_test_io_seek_shakedown() {
    ...
    {
        let mut rw_stream = File::create(filename).unwrap();
        let _ = rw_stream.write(initial_msg.as_bytes()).unwrap();
    }
    {
        let mut read_stream = check!(File::open(filename));
...
fn file_test_io_eof() {
    ...
    let mut buf = [0; 256];
    {
        let oo = OpenOptions::new()
            ...
            .read(true)
            .clone();
        let mut rw = oo.open(&filename).unwrap();
        assert_eq!(rw.read(&mut buf).unwrap(), 0);
        assert_eq!(rw.read(&mut buf).unwrap(), 0);
    }
    check!(fs::remove_file(&filename));
}
...
fn file_test_io_read_write_at() {
    ...
    {
        let mut read = File::open(&filename).unwrap();
        
        assert_eq!(read.read_at(&mut buf, 0).unwrap(), content.len());
        
        assert_eq!(str::from_utf8(&buf[..content.len()]), Ok(content));
        
        assert_eq!(read.seek(SeekFrom::Current(0)).unwrap(), 0);
        
        assert_eq!(read.seek(SeekFrom::End(-5)).unwrap(), 9);
        
        assert_eq!(read.read_at(&mut buf, 0).unwrap(), content.len());
        
        assert_eq!(str::from_utf8(&buf[..content.len()]), Ok(content));
        
        assert_eq!(read.seek(SeekFrom::Current(0)).unwrap(), 9);
        
        assert_eq!(read.read(&mut buf).unwrap(), write3.len());
        
        assert_eq!(str::from_utf8(&buf[..write3.len()]), Ok(write3));
        
        assert_eq!(read.seek(SeekFrom::Current(0)).unwrap(), 14);
        ...
        assert_eq!(read.read_at(&mut buf, 14).unwrap(), 0);
        
        assert_eq!(read.read_at(&mut buf, 15).unwrap(), 0);
...
fn binary_file() {
    ...
    let written = File::create(tmpdir.join("test"))
        .unwrap()
        .write(&bytes)
        .unwrap();
